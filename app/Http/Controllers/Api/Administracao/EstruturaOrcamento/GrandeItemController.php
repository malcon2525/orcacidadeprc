...
class GrandeItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $ordemExistente = GrandeItem::where('eo_tipo_orcamento_id', $request->eo_tipo_orcamento_id)
                ->where('ordem', $request->ordem)
                ->exists();

            if ($ordemExistente) {
                // Reordenar itens existentes
                GrandeItem::where('eo_tipo_orcamento_id', $request->eo_tipo_orcamento_id)
                    ->where('ordem', '>=', $request->ordem)
                    ->increment('ordem');
            }
        ...
        try {
            ...
            if ($ordemAntiga !== $novaOrdem) {
                if ($novaOrdem > $ordemAntiga) {
                    // Movendo para baixo
                    GrandeItem::where('eo_tipo_orcamento_id', $grandeItem->eo_tipo_orcamento_id)
    ...
    public function reordenar(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'itens.*.id' => 'required|exists:eo_grandes_itens,id',

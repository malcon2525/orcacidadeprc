<template>
    <div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body" v-show="filtrosVisiveis">
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        <div class="form-floating">
                            <select class="form-control" id="filtroModulo" v-model="filtros.modulo">
                                <option value="">Todos</option>
                                <option value="administracao">Administração</option>
                                <option value="orcamento">Orçamento</option>
                                <option value="transporte">Transporte</option>
                                <option value="tabela_oficial">Tabela Oficial</option>
                            </select>
        ...
        <div class="card">
            <div class="card-body p-0">
                    ...
                    <table class="table table-hover align-middle permissoes-table">
                        <thead>
                            <tr>
                                ...
                                <th class="fw-semibold text-custom">Módulo</th>
                        ...
                        <tbody>
                            <tr v-for="permissao in dados" :key="permissao.id" class="permissao-row">
                                <td class="align-middle">
                                    <div class="fw-medium">{{ permissao.name }}</div>
                                </td>
                                <td class="align-middle">
                                    <code class="text-muted">{{ permissao.slug }}</code>
                                </td>
                                <td class="align-middle">
                                    <span class="badge" :class="getModuloBadgeClass(permissao.module)">
                                        {{ getModuloLabel(permissao.module) }}
                                    </span>
                                </td>
                                <td class="align-middle">
                                    <div class="text-muted">{{ permissao.description || 'Sem descrição' }}</div>
                                </td>
                                <td class="align-middle">
                                ...
                                <td class="align-middle">
                                    <span class="badge bg-info">{{ permissao.roles_count || 0 }}</span>
                                </td>
                                <td class="text-end">
                                    <div class="d-flex gap-1 justify-content-end">
                                        <!-- Botão Editar: AMARELO SÓLIDO -->
                                        <button class="btn btn-sm btn-warning" @click="editarItem(permissao)" title="Editar">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <!-- Botão Excluir: VERMELHO SÓLIDO -->
                                        <button class="btn btn-sm btn-danger" @click="excluirItem(permissao)" title="Excluir">
                ...
                <div v-else class="text-center py-5">
                    ...
                    <h6 class="mt-3 text-muted">Nenhuma permissão encontrada</h6>
                    <p class="text-muted">Não há permissões cadastradas ainda.</p>
                </div>
            </div>
        </div>
        ...
        <!-- Modal de Formulário -->
        <div class="modal fade" id="modalPermissao" tabindex="-1" ref="modalPermissaoRef">
                    ...
                    <div class="modal-header custom-modal-header">
                        <div class="d-flex align-items-center">
                            ...
                            <h5 class="modal-title mb-0">
                                {{ modoEdicao ? 'Editar Permissão' : 'Nova Permissão' }}
                            </h5>
                        </div>
                        ...
                        <form @submit.prevent="salvarPermissao">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="invalid-feedback" v-if="errors.module">
                                        {{ errors.module[0] }}
                                    </div>
                                </div>
                    ...
                    <div class="modal-body">
                        <div class="text-center">
                            ...
                            <p class="text-muted mb-0">
                                <strong>"{{ itemParaExcluir?.name }}"</strong> será removida permanentemente.
                            </p>
    ...
    methods: {
        ...
        async carregarDados() {
            ...
            try {
                ...
                const response = await fetch(`/api/permissoes?${params}`);
        ...
        async confirmarExclusao() {
            ...
            this.excluindo = true;
            try {
                const response = await fetch(`/api/permissoes/${this.itemParaExcluir.id}`, {
                    method: 'DELETE',
                    headers: {
        ...
        getModuloLabel(modulo) {
            ...
            return labels[modulo] || 'Desconhecido';
        },
        
        getModuloBadgeClass(modulo) {
            const classes = {
                'administracao': 'bg-primary',
                'orcamento': 'bg-success',
                'transporte': 'bg-info',
                'tabela_oficial': 'bg-warning'
            };
            return classes[modulo] || 'bg-secondary';
        },

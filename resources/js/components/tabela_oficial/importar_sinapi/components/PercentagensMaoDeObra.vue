  ...
  <div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <h5 class="mb-1 fw-bold text-custom">
          <i class="fas fa-percentage me-2"></i>Processamento de Percentagens de Mão de Obra SINAPI
        </h5>
        <p class="text-muted mb-0 small">Processe percentagens de mão de obra com e sem desoneração da tabela SINAPI</p>
      </div>
    </div>
    ...
    <div class="row g-4 mb-4">
      ...
      <div class="col-md-6 col-lg-3">
        <WorkflowCard
          ...
          description="Selecione o Excel com percentagens de mão de obra"
      ...
      <div class="col-md-6 col-lg-3">
        <WorkflowCard
          ...
          title="Processamento"
          description="Inicie o processamento das percentagens"
          icon="fa-cogs"
          ...
          <template #default="{ status }">
            ...
            <div v-if="status === 'available'">
              <div class="processing-section">
                <div class="processing-info">
                  <p class="mb-3 small">Inicie o processamento das percentagens. Este processo pode levar alguns minutos.</p>
                </div>
                <button 
            ...
            <div v-else-if="status === 'processing'">
              <ResultDisplay
                type="processing"
                title="Processando..."
                description="Executando script Python e extraindo percentagens..."
              />
            </div>
            ...
            <div v-else-if="status === 'completed'">
              <ResultDisplay
                ...
                :description="`${stepStates.step2.data?.length || 0} percentagens extraídas`"
                :actions="[
          ...
          <template #default="{ status }">
            ...
            <div v-if="status === 'available'">
              <div class="validation-section">
                <div class="validation-info">
                  <p class="mb-3 small">Valide a estrutura e qualidade das percentagens extraídas antes de prosseguir.</p>
                </div>
                <button 
            ...
            <div v-else-if="status === 'completed'">
              <ResultDisplay
                ...
                :description="`${stepStates.step3.data?.validRecords || 0} percentagens validadas durante o processamento`"
                :actions="[]"
    ...
    <div v-if="currentProcessingStep" class="processing-details mb-4">
      <div class="processing-header">
        ...
        <div class="processing-info">
          <h6 class="mb-1 fw-semibold">Processando Percentagens de Mão de Obra</h6>
        ...
        <div class="modal-content border-0 shadow-lg">
          <div class="modal-header bg-white">
            <h5 class="modal-title fw-semibold text-custom" id="modalVisualizarDadosLabel">
              <i class="fas fa-table me-2"></i>Percentagens Processadas - {{ stepStates.step2.data?.length || 0 }} registros
            </h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body p-0">
            <div class="table-responsive">
              <table class="table table-hover mb-0 percentagens-mao-de-obra-table">
                <thead>
                  <tr>
                    <th>Código</th>
                    <th>Descrição</th>
                ...
                <tbody>
                  <tr v-for="(item, index) in stepStates.step2.data" :key="index">
                    ...
                    <td>{{ item.unidade }}</td>
                    <td class="text-end">{{ item.percentagem }}%</td>
                    <td>{{ item.tipo }}</td>
                  </tr>
                </tbody>
              </table>
...
export default {
    ...
    data() {
        return {
            ...
            etapas: [
                ...
                { nome: 'Processamento Excel', completa: false, ativa: false },
    ...
    methods: {
        ...
        validarArquivo(event) {
            ...
            if (!arquivo.type.includes('spreadsheet') && !arquivo.name.endsWith('.xlsx') && !arquivo.name.endsWith('.xls')) {
                this.arquivoValido = false
                this.nomeArquivo = arquivo.name
                this.mensagem = {
                    tipo: 'erro',
                    texto: 'Por favor, selecione um arquivo Excel válido (.xlsx ou .xls).'
                }
                return
            }
            ...
            this.mensagem = {
                tipo: 'sucesso',
                texto: 'Arquivo Excel válido selecionado.'
            }
        },
        ...
        async processarArquivo() {
            ...
            if (!this.arquivoValido) {
                this.mensagem = {
                    tipo: 'erro',
                    texto: 'Por favor, selecione um arquivo Excel válido primeiro.'
                }
                return
            }
            ...
            try {
                ...
                this.progressoEtapa = 'Validando arquivo Excel...'
                this.progresso = 10
                ...
                this.progressoEtapa = 'Executando script Python para extração de percentagens...'
                this.progresso = 30
                await this.delay(1000)
                
                this.progressoEtapa = 'Processando Excel com pandas...'
                this.progresso = 40
                await this.delay(1500)
                
                this.progressoEtapa = 'Extraindo percentagens das abas...'
                this.progresso = 50
                await this.delay(1000)
                
                // Etapa 3: Processando dados (60-80%)
                this.atualizarEtapa(3)
                this.progressoEtapa = 'Processando percentagens extraídas...'
                this.progresso = 60
                ...
                this.progressoEtapa = 'Percentagens processadas com sucesso!'
                this.progresso = 100
                await this.delay(500)
        ...
        async validarDados() {
            ...
            try {
                ...
                this.mensagem = {
                    tipo: 'sucesso',
                    texto: `Validação concluída! ${validRecords} percentagens válidas.`
                }
        ...
        handleAction(actionKey, data) {
            switch (actionKey) {
                case 'view':
                    ...
                    break
                case 'export':
                    this.exportarParaExcel(data, 'percentagens')
                    break
                case 'details':

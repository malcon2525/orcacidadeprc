...
const executarSincronizacao = async () => {
    try {
        ...
        if (response.data.data && response.data.data.resultados) {
...
const testarConexao = async () => {
    try {
        ...
        if (data.success) {
            ...
            if (window.showToast) {
                window.showToast('Erro na conexão: ' + data.message, 'error')
            }
            statusConexao.value = 'error'
        }
...
const salvarConfiguracao = async () => {
    ...
    try {
        ...
        if (response.data.data && response.data.data.updated_at) {
            ultimaAtualizacao.value = new Date(response.data.data.updated_at).toLocaleString('pt-BR')
        } else {

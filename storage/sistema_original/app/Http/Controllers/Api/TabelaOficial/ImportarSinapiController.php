...
class ImportarSinapiController extends Controller
{
    ...
    public function processarComposicoesInsumos(Request $request)
    {
        try {
            ...
            $caminhoAbsolutoEntrada = storage_path('app/' . $caminhoRelativoEntrada);
            ...
            $comando = "python " . base_path("01_python/importacao_SINAPI/01.Importar-SINAPI-Composicoes-Insumos.py") . " " . escapeshellarg($caminhoAbsolutoEntrada);
            $saida = shell_exec($comando);
            
            // Decodificar saída JSON
            $dados = json_decode($saida, true);
            
            if (json_last_error() !== JSON_ERROR_NONE) {
                $this->logErroCritico('SCRIPT_PYTHON', 'Erro ao decodificar JSON do script Python');
                throw new \Exception("Erro ao processar saída do script Python: " . json_last_error_msg());
            }
            
            $this->logProgresso('SCRIPT_PYTHON', 'Script Python executado com sucesso', [
                'total_arquivos_processados' => count($dados)
            ]);
    ...
    public function processarPercentagensMaoDeObra(Request $request)
    {
        try {
            ...
            if (!$nomeDiretorio) {
                // Fallback: procurar diretório mais recente
                $diretorios = glob(storage_path('app/temp/processado_sinapi_*'));
                if (empty($diretorios)) {
            ...
            $diretorioProcessamento = storage_path('app/temp/' . $nomeDiretorio);
    ...
    public function downloadArquivoProcessado(string $tipo)
    {
        try {
            ...
            $caminhoArquivo = storage_path('app/temp/' . $nomeDiretorio . '/' . $nomeArquivo);
